...
```bash
# Single relay
python3 probe-relay.py REDACTED-IP

# Include ICMP baseline and run 10 repeats
python3 probe-relay.py --icmp --repeat 10 REDACTED-IP

# Compare multiple relays side by side
python3 probe-relay.py --icmp REDACTED-IP REDACTED-IP REDACTED-IP

# Auth-required relay
python3 probe-relay.py --ticket-file /path/to/relay-ticket.txt REDACTED-IP
```
...
python3 relay-speedtest.py client \
  --relay-host REDACTED-IP \
  --target-host REDACTED-IP \
  --target-port 9000 \
  --payload-bytes 1200 \
  --upload-packets 4000 \
...
Quick local probe:
```bash
python3 probe-relay.py --icmp REDACTED-IP
```
